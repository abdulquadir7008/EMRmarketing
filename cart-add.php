...
//  $priceid = $listprod[ "price" ];
  if ($_SESSION[ 'member_id' ] ) {
    ...
    $qty_record = $cartsqlist[ 'qty' ] + 1;
    if ( $cartsqlist[ 'product_id' ] == $product_id && $cartsqlist[ 'userid' ] == $customerchechlogin_id ) {
      $queryatleast = "update cart SET qty='$qty_record' WHERE product_id=$product_id and userid='$customerchechlogin_id'";
      mysqli_query( $link, $queryatleast );
    } else {
      $query = "insert into cart(price,sess,qty,product_id,date,userid)values('$priceid','$sess','1','$product_id',now(),'$customerchechlogin_id')";
    ...
    if ( $cartsqlist[ 'product_id' ] == $product_id && $cartsqlist[ 'sess' ] == $sess) {
      $queryatleast = "update cart SET qty='$qty_record' WHERE product_id='$product_id' and sess='$sess'";
      mysqli_query( $link, $queryatleast );
    } else {
      $query = "insert into cart(price,sess,qty,product_id,date)values('$priceid','$sess','1','$product_id',now())";
      mysqli_query( $link, $query );
    }
