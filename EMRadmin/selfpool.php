...
if((isset($_GET['level']) && !empty($_GET['level'])) && (isset($_GET['level1']) && !empty($_GET['level1'])))
{
    $bif = $_GET['level'];
    $icd = $_GET['level1'];
    // $res=GetUsersIDSelfPool($db,$id,'85','pairing_1'); 
...
<html lang="en">
    ...
    <head>
        ...
        <meta charset="utf-8" />
        <title>Pool 2 Team | EMR Marketing</title>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-flex align-items-center justify-content-between">
                                    <h4 class="mb-0">Pool 2 Team </h4>

                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">CMS</a></li>
                                            <li class="breadcrumb-item active">Pool 2 Team </li>
                                        </ol>
                                    </div>
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="table-responsive mb-4">
                                        ...
                                        <form name="LevelFilter" action="" method="get" class="row row-cols-lg-auto align-items-center">
                                            <div class="col-12 col-lg-4">
                                                ...
                                                <?php
                                                    ...
                                                    // FROM `child_counter_1` t1 where t1.uid LIKE '$customeid%' ");
                                            ...
                                            <div class="col-12 col-lg-4">
                                                ...
                                                <select class = "form-control form-select" name = "level1" id = "level1" required>
                                                <?php
                                                    $id = 1;
                                                    $sql=mysqli_query($db,"SELECT t1.*,t1.id as idd
                                                    FROM `child_counter_1` t1 where t1.uid LIKE '$bif%' ");
                                                    while($row = mysqli_fetch_assoc($sql))
                                                    { 
                                    ...
                                    <table class="table table-centered datatable dt-responsive nowrap table-card-list" style="border-collapse: collapse; border-spacing: 0 12px; width: 100%;">
                                        ...
                                        $count = 0;
                                        // $i = 1;
                                        // $sql=mysqli_query($db,"select t1.*,t2.* from pairing_1 t1 join membership t2 on t2.member_id = t1.uid  where t1.uid='$customeid' and t1.pair_id != 1 ORDER BY t1.pair_id ASC");
                                        // $row1 = mysqli_fetch_assoc($sql);
                                        // $pair_id = $row1['pair_id'];
                                        // if($pair_id != ''){
                                        // $brnadSQL="select t1.*,t1.date as dt,t2.* from pairing_1 t1 join membership t2 on t2.member_id = t1.uid where t1.pair_id>'$pair_id'"; 
                                        // $ResultSQL=mysqli_query($link,$brnadSQL); 
                                        // while($Listbrand=mysqli_fetch_array($ResultSQL)) { 
                                        if($icd != '0'){
                                        for ($ci=1; $ci < 12; $ci++) { 
                                            $sql=mysqli_query($db,"select t1.* from child_counter_1 t1 where t1.uid='$icd'");
                                            $row1 = mysqli_fetch_assoc($sql);
                                            $res=GetUsersIDPool($db,$ci,$icd,'pairing_1'); 
                                            $count = count($res); 
                                            $decimalNumber = $icd;
                                            $integerPart = floor($decimalNumber);
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if($ci == 1){echo $amt = "50";}elseif($ci == 2){echo $amt = "100";}elseif($ci == 3){echo $amt = "200";}elseif($ci == 4){echo $amt = "400";}elseif($ci == 5){echo $amt = "800";}elseif($ci == 6){echo $amt = "525";}elseif($ci == 7){echo $amt = "375";}elseif($ci == 8){echo $amt = "750";}elseif($ci == 9){echo $amt = "1500";}elseif($ci == 10){echo $amt = "2325";}elseif($ci == 11){echo $amt = "3675";}?> </td>
                                            <td><?php $slot_count=mysqli_num_rows(mysqli_query($db,"SELECT * FROM `selfpool_income` WHERE `uid`='$integerPart' and `fuid`='$row1[id]' and `amount` = '$amt'"));
                                            if(($slot_count) >0)
                                            { 
                                                echo "Completed";
                                            }
