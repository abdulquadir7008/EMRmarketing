...
require '../vendor_email/autoload.php';
	...
	$userc04=mysqli_num_rows(mysqli_query($db,"select * from membership where userid='$_REQUEST[spnoser_id]'"));
	if($userc04 > 0){
		...
		{	
			...
			if(mysqli_num_rows($result)==0)
			{
				if(mysqli_num_rows($result_sponser)==0)
				{
					...
					$query="insert into membership(fname,lname,phone,email,country,stree_address,city,state,postalcode,status,password,date,sess,gender,sponser_status,spnoser_id,binary_root,binary_status)values('$fname','$lname','$phone','$email','$country','$stree_address','$city','$state','$postalcode','$sess','$password',now(),'$sess','$gender','$sponser_status','$spnoser_id','$act_pos','$bin')"; 
		...
		// $errmsg_arr[] = 'Error: Sponsor ID is In-Valid.';
		$errmsg_arr[] = '<div class="alert alert-danger alert-dismissable"> <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a> <strong>Error!</strong> Sponsor ID is In-Valid!</div>';
		$errflag = true;
		$_SESSION['registerpro'] = $errmsg_arr;
		session_write_close();
		header('Location: ' . $_SERVER['HTTP_REFERER']);
